...
localParseTrackLimit = 200; % <---edit: keep Inf to avoid truncating true left-moving population
...
for i = 1:numel(cases)
    ...
    fprintf(['Strict gate summary: total=%d, injected=%d, activated=%d, ', ...
        'reject(originBox=%d short=%d nonfinite=%d nonmonoT=%d topo=%d flow=%d origin=%d noAct=%d wall=%d)\n'], ...
...
policyTag = sprintf([ ...
    ...
    'spots=%d|gaps=%.4g|rsmc=%d|wall=%d|oxOn=%d|ox0=%.4g|ox1=%.4g|oy0=%.4g|oy1=%.4g|jump=%.4g|pre=%d|minpre=%d|post=%d|', ...
    ...
    double(logical(qcOpts.wallBandEnabled)), ...
