...
localParseTrackLimit = 50; % <---edit: keep Inf to avoid truncating true left-moving population
...
lagAccelOpts.heatmapStats = "mean";
lagAccelOpts.makePooledHeatmapPlots = false; % separate roughness heatmaps are easier to interpret
lagAccelOpts.makeCaseHeatmapPlots = true;
...
lagAccelOpts.activationOverlayPerCase = 75; % plot-only marker cap; activationXYNorm saved in .mat is not capped. Set Inf when replotting to draw all.
...
cases(1).xmlFile   = "E:\March Re 90,000 inception data\Processed images\Batch data 2\results1\Smooth"; % <---edit: set your XML path
...
cases(2).Re        = 95000;
cases(2).kD       = 0.080;
cases(2).xmlFile   = "E:\March Re 90,000 inception data\Processed images\Batch data 2\results1\P10S100";
...
cases(3).Re        = 95000;
cases(3).kD       = 0.141;
cases(3).xmlFile   = "E:\March Re 90,000 inception data\Processed images\Batch data 2\results1\P10S70";
...
cases(4).Re        = 95000;
cases(4).kD       = 0.267;
cases(4).xmlFile   = "E:\March Re 90,000 inception data\Processed images\Batch data 2\results1\P10S50";
...
cases(5).Re        = 95000;
cases(5).kD       = 0.444;
cases(5).xmlFile   = "E:\March Re 90,000 inception data\Processed images\Batch data 2\results1\P10S30";
...
cases(6).Re        = 95000;
cases(6).kD       = 0.720;
cases(6).xmlFile   = "E:\March Re 90,000 inception data\Processed images\Batch data 2\results1\P10S20";
...
% See analyze_breakup_events.m for full signal definition.
...
for i = 1:numel(cases)
    ...
    tau_median_val = activation_tau_median(metrics);
    ...
    activatedVelocity_mean_m_s = metrics.activatedUpstreamVelocity_mean_m_s;
    activatedVelocity_std_m_s = metrics.activatedUpstreamVelocity_std_m_s;
    activatedVelocity_sem_m_s = metrics.activatedUpstreamVelocity_sem_m_s;
    activatedVelocity_n = metrics.activatedUpstreamVelocity_n;
    ...
    A_over_I_absolute = activation_absolute_ai(metrics, nTotalAE, nStrictRecirculationTracks);
    ...
    row = table( ...
        ...
        metrics.A_over_I, metrics.A_over_I_ci_low, metrics.A_over_I_ci_high, A_over_I_err_low, A_over_I_err_high, A_over_I_absolute, ...
        tau_mean_val, tau_median_val, tau_std_val, tau_sem, nTauVals, ...
        ...
        'leftMovingFrac_total','activationFrac_valid','A_over_I','A_over_I_ci_low','A_over_I_ci_high','A_over_I_err_low','A_over_I_err_high','A_over_I_absolute', ...
        'tau_mean','Tau_median','tau_std','tau_sem','nTau', ...
        'activatedVelocity_mean_m_s','activatedVelocity_std_m_s','activatedVelocity_sem_m_s','activatedVelocity_n', ...
        'elapsed_case_sec'});
...
summaryRow = table( ...
    ...
    strictRecirculationFrac_total, strictActivationFrac_valid, ...
    metrics.A_over_I, metrics.A_over_I_ci_low, metrics.A_over_I_ci_high, A_over_I_err_low, A_over_I_err_high, A_over_I_absolute, ...
    metrics.tau_mean, tau_median_val, metrics.tau_std, tau_sem, nTauVals, ...
...
function val = activation_tau_median(metrics)
if isfield(metrics, 'tau_median') && ~isempty(metrics.tau_median) && ...
        isscalar(metrics.tau_median) && isfinite(metrics.tau_median)
    val = metrics.tau_median;
elseif isfield(metrics, 'tau_values')
    tauVals = metrics.tau_values(:);
...
function val = activation_absolute_ai(metrics, nTotalAE, nLeftMovingTracks)
if isfield(metrics, 'A_over_I_absolute') && ~isempty(metrics.A_over_I_absolute) && ...
        isscalar(metrics.A_over_I_absolute) && isfinite(metrics.A_over_I_absolute)
    val = metrics.A_over_I_absolute;
elseif isfinite(nTotalAE) && isfinite(nLeftMovingTracks) && nLeftMovingTracks > 0
    val = nTotalAE / nLeftMovingTracks;
else
    val = NaN;
end
end
